...
final _authReducer = combineReducers<AuthState?>([
  TypedReducer<AuthState?, LoginRequest>(_login),
]);

AuthState? _login(AuthState? state, LoginRequest request) {
...
List<IncomingMatch> _saveBets(List<IncomingMatch> matches,
    SaveBetsAction action,) {
  return matches
      .map((match) =>
  action.bets.containsKey(match.matchId)
...
Map<String, MatchScore> _changeBet(Map<String, MatchScore> bets,
    ChangeBetAction action) {
  return Map.unmodifiable({}
    ..addAll(bets)
    ..update(action.matchId, (_) => action.bet, ifAbsent: () => action.bet));
}

Map<String, MatchScore> _resetBet(Map<String, MatchScore> bets,
    ResetBetAction action) {
  return Map.unmodifiable({}
    ..addAll(bets)
    ..removeWhere((key, _) => key == action.matchId));
}

Map<String, MatchScore> _resetAllBets(Map<String, MatchScore> bets,
    ResetBetsAction action) {
  return Map.unmodifiable({});
}

Map<String, MatchScore> _resetAllBetsOnSave(Map<String, MatchScore> bets,
    SaveBetsAction action) {
  return Map.unmodifiable({});
}
